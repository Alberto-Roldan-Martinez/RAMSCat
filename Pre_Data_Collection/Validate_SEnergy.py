...
def Area(element, x, coord_matrix, popt_lineal, notes_validation):
	y = []
	note = []
	for i in range(len(x)):
		note.append(notes(notes_validation[i]))
		surf_coord = [int(coord_matrix[i][j]) for j in range(len(coord_matrix[i]))]
		y.append(sum([trendline_2(j, *popt_lineal) for j in surf_coord]))          # in Angstroms^2
