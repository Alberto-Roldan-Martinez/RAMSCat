...
#				('MgO', 'Au',  9,  ),		# 10 atoms
	...
	return i_adh_e, reference_e, e_min, [a2, b2]
...
for i in cluster_interface_indexes:
	...
	adh_e, reference_e, e_min, distances_opt = morse_3D_Energies(support_name, supported_cluster[int(i)].symbol,
											len(atom_cluster_neighbours[str(i)]), distance_a[0][1], distance_b[0][1])

	cluster_interface_adh_e.append([i, round(adh_e, 5), round(e_min, 5), round(distance_a[0][1]/distances_opt[0], 3),
...
primary_cluster_sites = [cluster_interface_adh_e[0][0]]
secondary_cluster_sites = []
predicted_adhesion_e = 0
for n in range(1, len(cluster_interface_adh_e)):
	i = cluster_interface_adh_e[n][0]
	if i not in secondary_cluster_sites or cluster_interface_adh_e[n][1] < cluster_interface_adh_e[n][2]*0.60: 				# 0.60 << GOOD
		if cluster_interface_adh_e[n][1] < cluster_interface_adh_e[0][1]*0.70:												# 0.70
...
if len(cluster_interface_adh_e) == len(primary_cluster_sites):
	for n in range(1, len(cluster_interface_adh_e)):
		i = cluster_interface_adh_e[n][0]
		if cluster_interface_adh_e[n][3]/cluster_interface_adh_e[n][4] > 1.0:
			primary_cluster_sites.remove(i)
			secondary_cluster_sites.append(i)
secondary_cluster_sites = set([cluster_interface_adh_e[i][0] for i in range(len(cluster_interface_adh_e)) if cluster_interface_adh_e[i][0] not in primary_cluster_sites])
